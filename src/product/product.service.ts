...
export class ProductService {
    ...
    async createProduct(user: any, createProductDto: CreateProductDto) {
        ...
        if (![Role.super_admin, Role.admin, Role.manager, Role.employee].includes(user.role)) {
            throw new ForbiddenException('Only admins, managers and emplyees can create products');
        }

        // Verify user has access to the store
        const storeAccess = user.stores?.find(store => store.storeId === createProductDto.storeId);
        ...
        try {
            ...
            const result = await this.prisma.$transaction(async (prisma) => {
                ...
                if (createProductDto.purchaseQuantity && createProductDto.purchasePrice) {
                    ...
                    if (employee) {
                        purchaseOrder = await prisma.purchaseOrders.create({
                            data: {
                                ...
                                total: createProductDto.purchaseQuantity * createProductDto.purchasePrice,
    ...
    async updateProduct(user: any, productId: string, updateProductDto: UpdateProductDto) {
        // Check permissions
        if (![Role.super_admin, Role.admin, Role.manager, Role.employee].includes(user.role)) {
            throw new ForbiddenException('Only admins and managers can update products');
        }
        ...
        try {
            ...
            if (updateProductDto.storeId && updateProductDto.storeId !== existingProduct.storeId) {
                const storeAccess = user.stores?.find(store => store.storeId === updateProductDto.storeId);
                if (!storeAccess && user.role !== Role.super_admin) {
            ...
            const updatedProduct = await this.prisma.products.update({
                ...
                data: {
                    ...
                    ...(updateProductDto.storeId && { storeId: updateProductDto.storeId }),
                },
                select: {
                    id: true,
                    name: true,
